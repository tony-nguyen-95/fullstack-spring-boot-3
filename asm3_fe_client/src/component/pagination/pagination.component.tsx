...
export const Pagination: React.FC<Props> = observer(() => {
  const total = productStore.lengthOfProducts || 0;

  // Calculate the number of pages
  const numberOfPage = useMemo(() => {
    return Math.ceil(total / productStore.sizeOfPages);
  }, [total, productStore.sizeOfPages]);

  // Handle change in the number of items per page
  const handleChangeSize = (e: React.ChangeEvent<HTMLSelectElement>) => {
    productStore.setPageSize(parseInt(e.target.value, 10));
    productStore.setCurrentPage(1); // Reset to the first page when the size changes
  };

  // Handle page change
  const handlePageChange = (newPage: number) => {
    if (newPage > 0 && newPage <= numberOfPage) {
      productStore.setCurrentPage(newPage);
    }
  };
  ...
  return (
    <nav className="flex flex-wrap justify-between items-end mb-4">
      ...
      <form className="max-w-sm">
        ...
        <select
          id="unitPerPage"
          defaultValue={productStore.sizeOfPages}
      ...
      <ul className="inline-flex -space-x-px text-sm">
        <li>
          <div
            onClick={() => handlePageChange(productStore.currentPage - 1)}
